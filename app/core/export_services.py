...
class ExportService:
    ...
    def generate_pdf(self, tracking_records: List[TrackingRecord], include_details: bool = True) -> str:
        """
        Generate PDF report from tracking records
        ...
        try:
            ...
            # Table data
            if include_details:
                data = [['Tracking #', 'Status', 'Origin', 'Destination', 'Last Checked']]
                ...
                data = [['Tracking #', 'Status', 'Last Checked']]
                for record in tracking_records:
                    data.append([
                        record.tracking_number,
                        record.status or 'N/A',
    ...
    def generate_docx(self, tracking_records: List[TrackingRecord], include_details: bool = True) -> str:
        """
        Generate DOCX report from tracking records
        ...
        try:
            ...
            if include_details:
                ...
                headers = ['Tracking #', 'Status', 'Origin', 'Destination', 'Last Checked']
                ...
                for record in tracking_records:
                    ...
                    row_cells[1].text = record.status or 'N/A'
                    row_cells[2].text = record.origin or 'N/A'
                    row_cells[3].text = record.destination or 'N/A'
                    row_cells[4].text = record.last_checked.strftime('%Y-%m-%d %H:%M') if record.last_checked else 'N/A'
            else:
                ...
                # Header row
                header_cells = table.rows[0].cells
                headers = ['Tracking #', 'Status', 'Last Checked']
                for idx, header in enumerate(headers):
                    cell = header_cells[idx]
                    cell.text = header
                    cell.paragraphs[0].runs[0].font.bold = True
                ...
                for record in tracking_records:
                    ...
                    row_cells[0].text = record.tracking_number
                    row_cells[1].text = record.status or 'N/A'
                    row_cells[2].text = record.last_checked.strftime('%Y-%m-%d %H:%M') if record.last_checked else 'N/A'
